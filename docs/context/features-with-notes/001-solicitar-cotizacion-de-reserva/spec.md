...
> **Nota de trazabilidad**: Este caso de uso es invocado directamente por el Sistema de Reservas y Operaciones (Módulo de Reservas) para calcular el costo estimado de reservas —en lote o de forma individual— delegando íntegramente la responsabilidad matemática al sistema. El sistema incluye obligatoriamente (`<<include>>`) a "Brindar tarifa base" (SPEC 2) para obtener la tarifa vigente de cada embarcación consultada al Sistema de Gestión de Flota, y asume una duración por defecto de 1 día y 1 pasajero cuando no se especifican fechas exactas. El desglose completo de precio (incluyendo seguro náutico y depósito de garantía) que sí es devuelto al Sistema de Reservas corresponde a "Solicitar el valor calculado de la reserva" (SPEC 4); este caso de uso solo devuelve la cotización estimada y su advertencia sobre la naturaleza del estimado, sin persistir ningún registro financiero.
