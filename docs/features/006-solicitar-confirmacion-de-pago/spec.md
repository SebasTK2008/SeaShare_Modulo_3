...
Como el sistema, al recibir del Sistema de Reservas y Operaciones una solicitud de confirmación de pago para una reserva específica, quiero consultar el registro de cobro previamente creado por "Procesar cobro" para esa reserva, de manera que pueda devolver el estado actual de la operación (en proceso, aprobado, rechazado, cancelado, expirado o desconocido), junto con sus montos y referencias cuando estén disponibles.
...
**Prueba Independiente**: Con un registro de cobro previamente creado por "Procesar cobro" en distintos estados (en proceso, aprobado, rechazado, cancelado, expirado y desconocido), enviar al sistema una solicitud de confirmación de pago para esa reserva y validar que devuelve el estado correspondiente junto con los montos y referencias disponibles.

**Escenarios de Aceptación**:

1. **Escenario**: Confirmación de un cobro exitoso.
  - **Dado** que el registro de cobro de una reserva fue marcado como aprobado por "Procesar cobro".
   - **Cuando** el Sistema de Reservas y Operaciones solicita la confirmación de pago de esa reserva.
  - **Entonces** el sistema devuelve el estado aprobado junto con el monto autorizado o cobrado y la referencia externa provista por la Pasarela de pago. Este estado no implica por sí mismo que la captura o liquidación posterior ya se haya ejecutado.
...
2. **Escenario**: Confirmación de un cobro fallido.
   ...
   - **Cuando** el Sistema de Reservas y Operaciones solicita la confirmación de pago de esa reserva.
  - **Entonces** el sistema devuelve el estado externo correspondiente y su detalle, sin reportar un cobro aprobado cuando la pasarela lo rechazó, canceló o dejó expirado.
...
3. **Escenario**: Consulta de un cobro que aún está en proceso.
   ...
   - **Cuando** el Sistema de Reservas y Operaciones solicita la confirmación de pago de esa reserva.
  - **Entonces** el sistema devuelve el estado "en proceso", sin inventar un resultado definitivo de aprobación o rechazo.
...
- **¿Qué sucede si se solicita la confirmación de pago mientras el registro de cobro aún se encuentra en estado "en proceso"?**
  Según RF-003, el sistema devuelve fielmente el estado "en proceso" registrado, sin inventar ni anticipar un resultado de aprobación o rechazo que la Pasarela de pago aún no ha reportado.
...
- **RF-003**: El sistema DEBE devolver el estado vigente de la operación registrada (en proceso, aprobado, rechazado, cancelado, expirado o desconocido), junto con su detalle cuando esté disponible.
- **RF-004**: El sistema DEBE incluir los montos autorizado, capturado, liberado o cobrado y las referencias externas cuando estén disponibles.
...
- **CE-001**: Trazabilidad, "100% de las solicitudes de confirmación de pago devuelven un estado (en proceso, aprobado, rechazado, cancelado, expirado o desconocido) que coincide exactamente con el registro de cobro creado por 'Procesar cobro', con cero (0) discrepancias detectadas en pruebas automatizadas".
