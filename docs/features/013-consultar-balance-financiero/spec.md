...
- **RF-006**: El sistema DEBE calcular los depósitos en garantía pendientes como la suma de los depósitos fijos capturados de reservas completadas que, a la fecha de fin del período seleccionado, no cuenten con un reembolso o liquidación de garantía exitoso registrado. Una disputa `PENDIENTE` o `RECHAZADO` no cierra por sí sola el ciclo financiero del depósito.
...
- **InformaciónDeReserva (Entidad, definida en SPEC 3 y actualizada en SPEC 4)**: En este caso de uso es únicamente consultada para identificar depósitos fijos capturados y reservas completadas cuyo ciclo financiero de garantía permanece abierto.
