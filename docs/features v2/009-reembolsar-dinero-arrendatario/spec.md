...
> **Nota de trazabilidad**: Este caso de uso es invocado internamente por dos operaciones distintas del sistema, a través de tres orígenes posibles: "Brindar el estado de la reserva" (SPEC 7), cuando el estado informado corresponde a una **cancelación flexible**, una **cancelación moderada**, o a la **finalización de la reserva sin incidentes** ("completada sin incidentes"); y "Resolver disputa de garantía" (SPEC 8), mediante una relación `<<extend>>`, cuando el resultado de la disputa —evaluada por el Administrador Financiero tras una reserva informada como **"completada con incidentes"**— favorece total o parcialmente al arrendatario. El sistema no expone este caso de uso a ningún actor externo: es la Pasarela de Pago quien ejecuta técnicamente la devolución del monto determinado y reporta su resultado. Para las cancelaciones dentro de la ventana flexible (>72h), el sistema únicamente solicita a la Pasarela de Pago la devolución del 100% del valor de alquiler previamente registrado; es la propia Pasarela de Pago quien, al ejecutar la devolución, neta sus propios costos de procesamiento transaccional — dicha deducción es una condición externa de la Pasarela de Pago y no un cálculo realizado por el sistema.
